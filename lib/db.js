...
exports.saveHerding = async function (records, isHerding = false) {
  ...
  const ret = await db.run(q, isHerding, records.map(r => r.id));
...
exports.savePixelData = async function (records) {
  if (!db) await setup();
  const now = performance.now();
  const values = {};
  ...
  if (settings.benchmark) console.log("Benchmark DB#savePixelData: ", performance.now() - now);
  return ret;
}
exports.insertUserGame = async function(sessionKey, group_id, group_order, isBot, isActive) {
  if (!db) await setup();
  const now = performance.now();
  ...
  const ret = await db.run(q, sessionKey, group_id, group_order, isBot, isActive);
  if (settings.benchmark) console.log("Benchmark DB#insertUserGame: ", performance.now() - now);
  return ret;
}
exports.insertNPCGame = async function(sessionKey, group_id, group_order) {
  return await exports.insertUserGame(sessionKey, group_id, group_order, true, true);
}

...
exports.insertUserGame = async function(sessionKey, username, group_id, group_order, isBot, isActive) {
  if (!db) await setup();
  const now = performance.now();
  ...
  const ret = await db.run(q, sessionKey, username, group_id, group_order, isBot, isActive);
...
exports.insertNPCGame = async function(sessionKey, username, group_id, group_order) {
  return await exports.insertUserGame(sessionKey, username, group_id, group_order, true, true);
}
